...
class AIMDFragmentation(object):
    ...
    def getjobname(self, *molid):
        if len(molid) == 1:
            return "mol{}".format(*molid)
        elif len(molid) == 2:
            molid1, molid2 = sorted(molid)
            return 'tb{}-{}'.format(molid1, molid2)
    ...
    def printgjf(self, jobname, selected_atoms1, S1, selected_atoms2=None, S2=None):
        ...
        buff = []
        selected_atoms, Stotal, kbodykeyword = (selected_atoms1, S1, self.onebodykeyword) if not selected_atoms2 else (
            selected_atoms1+selected_atoms2, S1+S2-1, self.twobodykeyword)
        selected_atoms.wrap(
            center=selected_atoms[0].position/selected_atoms.get_cell_lengths_and_angles()[0:3], pbc=selected_atoms.get_pbc())
        buff.append('%nproc={}\n%mem={}\n# force {}/{} {} {}\n\n{}\n\n0 {}'.format(self.nproc,
                                                                                   self.qmmem, self.qmmethod, self.qmbasis, kbodykeyword, self.addkw, jobname, Stotal))
    ...
    def printmol(self):
        ...
        for molid, atoms in enumerate(self.mols, 1):
            ...
            S = 3 if selected_atoms.get_chemical_symbols() == [
                'O', 'O'] else (Counter(selected_atoms.get_chemical_symbols())['H'] % 2+1)
            self.Smol.append(S)
        ...
        for molid1, atoms1 in enumerate(self.mols, 1):
            for molid2, atoms2 in enumerate(self.mols[molid1:], molid1+1):
                if np.min(get_distances(self.atoms[atoms1].positions, self.atoms[atoms2].positions, cell=self.atoms.get_cell(), pbc=self.atoms.get_pbc())[1]) <= self.cutoff:
                    ...
                    self.printgjf(jobname, selected_atoms1,
                                  S1, selected_atoms2, S2)
                    self.jobs.append(jobname)
    ...
    def readforce(self, jobname):
        ...
        if forces:
            ...
            for index, force in forces.items():
                atoms[self.atomid[jobname][index-1]
    ...
    def takeforce(self):
        onebodyforce, twobodyforce = np.zeros(
            (self.natom, 3)), np.zeros((self.natom, 3))
        with Pool(self.nproc_sum) as pool:
            onebodyresults = pool.imap(
                self.readforce, [self.getjobname(i) for i in range(1, len(self.mols)+1)])
            twobodyresults = pool.imap(self.readforce, [self.getjobname(i, j) for i in range(1, len(
                self.mols)+1) for j in range(i+1, len(self.mols)+1) if self.getjobname(i, j) in self.jobs])
            twobodyerroratoms = []
